 ...
 *
 */
public class EFGBasedTestCaseExecutionStrategy extends AbstractStrategy {
  ...
  public void run(String[] mainArgs) {
    ...
    String filterListFilePath = new File(TestProperty.getFolder("project"),
        TestProperty.propertyStore.get("termination_event_filename", "")).getAbsolutePath();
  ...
  private void testCaseExecution() {
    ...
    TestLogger.info("##@@ Start test case execution (index: %d)##@@", testCaseIndex);
    timeCounter.reset();
    timeCounter.start();
    // run program
    appManager.runApp();
    ...
    ArrayList<EventNode> testCase = (ArrayList<EventNode>) IOUtil.loadObjectFromXML(TestProperty.propertyStore
        .get("testcase_path"));
    testCaseLength = testCase.size();

    for (EventNode nextEventToExecute : testCase) {
      localCount++;
      ...
      if (nextEvent == null) {
        TestLogger.info("  Event index %d : %s(%s) doesn't exist!", localCount, nextEventToExecute.getName(),
        ...
        TestLogger.info("  Event index %d : %s(%s) is found...", localCount, nextEventToExecute.getName(),
            nextEventToExecute.getHashCode());
      }
  ...
  private void initialize(String[] mainArgs) {
    ...
    accumulatedCoverage = new ProjectData();
    ...
    testCaseIndex = extractTestIndex(TestProperty.propertyStore.get("testcase_path"));
    localCount = 0;
  ...
  private int extractTestIndex(String testCasePath) {
    int pivot = testCasePath.lastIndexOf("_");
    String numStr = testCasePath.substring(pivot + 1, pivot + 9);
